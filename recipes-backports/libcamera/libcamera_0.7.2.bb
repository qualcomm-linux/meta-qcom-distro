...
# Replaces the meta-multimedia recipe, only offer it to qcom-distro
python () {
    if 'qcom-distro' not in d.getVar('DISTROOVERRIDES').split(':'):
        raise bb.parse.SkipRecipe("libcamera backport is only used by qcom-distro")
}
